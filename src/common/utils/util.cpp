...
struct FilePath {
    ...
    FilePath(string filePath, string directoryPrefix) :
        filePath(filePath), directoryPrefix(directoryPrefix) {
    }
};
    ...
    while (1) {
        ...
        if (currEntry->d_name[0] == '.') {
            ...
            case DT_DIR:
                ...
                break;
            case DT_REG:
                files->push(FilePath(name,prefix));
                break;
            default:
...
foreachEntryInDirectory(const std::string& path,
    ...
    while (!directories.empty() || !files.empty()) {
        while (!files.empty()) {
            const FilePath file = files.front();
            files.pop();
            FAIL_ON(fileCallback(path + "/" + file.filePath,
                                 file.directoryPrefix) != 0);
        }
